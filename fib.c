...
int main(int argument_count, char *argument_values[]) {
   ...
   //initialize memoization
    ...
    computation_method = argument_values[2][0];//[0] is for accessign character in string, 'i' / 'r'
...
// iternative
long long compute_fibonacci_iterative(int fibonacci_index) {
    if (fibonacci_index <= 0) return 0;
    if (fibonacci_index == 1) return 1;

    if (memoization_array_iterative[fibonacci_index] != -1 && fibonacci_index < MAX_MEMO_SIZE) {
    ...
    for (int iteration_index = 2; iteration_index <= fibonacci_index; iteration_index++) {
...
long long compute_fibonacci_recursive(int fibonacci_index) {
    ...
    if (memoization_array_recursive[fibonacci_index] != -1 && fibonacci_index < MAX_MEMO_SIZE) {
